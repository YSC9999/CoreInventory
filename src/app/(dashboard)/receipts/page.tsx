...
import { receiptsData } from "@/lib/mock-data";
...
export default function Receipts() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'Waiting': return <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20 hover:bg-amber-500/20">Waiting</Badge>;
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">Inbound Receipts</h1>
        ...
        <Table>
          ...
          <TableBody>
            {receiptsData.map((receipt) => (
              <TableRow key={receipt.id} className="stagger-row border-border hover:bg-primary/5 transition-colors">
                <TableCell className="font-mono font-medium text-foreground py-4">{receipt.id}</TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    {receipt.status === 'Ready' && (
                      <Button size="sm" className="h-8 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg shadow-sm">
